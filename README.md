...
- **Closed type:** `type Decision = Route(Backend) | Deny` where `Backend` is closed
- **Sink / capability:** `grant routeApi` → `privileged { routeApi(d) }`
- **Consequence axes:** Trust

## Run the demo

```sh
examples/apps/api-route-gate/demo.sh
```
...
- `api-route-gate_safe.eide` — the correct design.
- `api-route-gate_unsafe.eide` — the tempting-but-wrong version (the negative example a model must learn to reject).
